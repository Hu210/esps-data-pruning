...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train_acc, train_los, loss_epoch, output_epoch, index_epoch, y_epoch = train(train_loader, args, net, criterion, optimizer, scheduler, epoch, log)
        ...
        }, is_best, args.save_path, f'best_ckpt.pth.tar')
        ...
        if epoch+1 == args.epochs:
            ...
            'recorder': recorder,
            'optimizer': optimizer.state_dict(),
        }, is_best, args.save_path, f'last_ckpt.pth.tar')
        ...
        if args.dynamics:
            dynamics_path = os.path.join(args.save_path, 'npy')
            ...
            np.save(os.path.join(dynamics_path, f"{epoch}_Output.npy"), output_epoch)
...
def train(train_loader, args, model, criterion, optimizer, scheduler, epoch, log):
    ...
    for t, (input, target) in enumerate(train_loader):
        ...
        if t==0:
            ...
            y_epoch = np.concatenate((y_epoch, np.array(y.detach().cpu())), axis = 0)
    ...
    return top1.avg, losses.avg, loss_epoch, output_epoch, index_epoch, y_epoch
